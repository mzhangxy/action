...
功能：使用单个Cookie字符串自动续约服务器，并验证续期是否成功
环境变量：
  - CASTLE_COOKIES : Cookie字符串（必需）
  - SERVER_ID : 服务器ID（默认为117954）
  - TG_BOT_TOKEN, TG_CHAT_ID : Telegram通知（可选）
"""
        ...
        for element in elements:
            try:
                ...
                if text and re.search(r'\d{2}\.\d{2}\.\d{4}', text):
                    ...
                    if match:
                        logger.info(f"📅 从元素提取到到期时间: {match.group(1)}")
